...
startButton.addEventListener('click', () => {
  if (isTimerStarted) return; // Не запускать, если таймер уже работает

  isTimerStarted = true;
  let counter = 3;
  countdownDisplay.textContent = counter; // Немедленно показать 3
...
cancelButton.addEventListener('click', () => {
  if (!isTimerStarted) return; // Таймер не запущен — ничего не делать
